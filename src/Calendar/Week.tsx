...
const isDaySelected = (
    ...
    if (isMoment(selectedDate)) {
        isSelected = selectedDate.isSame(
            startOfIsoWeek.isoWeekday(weekDay),
            'day'
        );
    } else {
        if (
            'start' in selectedDate &&
            isMoment(selectedDate.start) &&
            'end' in selectedDate &&
            isMoment(selectedDate.end)
        ) {
            isSelected =
                selectedDate.start.isSame(
                    startOfIsoWeek.isoWeekday(weekDay),
                    'day'
                ) ||
                selectedDate.end.isSame(
                    startOfIsoWeek.isoWeekday(weekDay),
                    'day'
                );
        } else if ('start' in selectedDate && isMoment(selectedDate.start)) {
            isSelected = selectedDate.start.isSame(
                startOfIsoWeek.isoWeekday(weekDay),
                'day'
            );
        }
...
const isDayInRange = (
    ...
    if (isDateRange(selectedDate) && selectedDate.start && selectedDate.end) {
        return startOfIsoWeek
            .isoWeekday(weekDay)
            .isBetween(selectedDate.start, selectedDate.end, 'day');
    }
    return false;
};
...
const IsInPotentialRange = (
    potentialRange: DateRange,
    startOfIsoWeek: moment.Moment,
    weekDay: number
) => {
    if (
        potentialRange.start &&
        potentialRange.end &&
        startOfIsoWeek
            .isoWeekday(weekDay)
            .isBetween(potentialRange.start, potentialRange.end, 'day')
    ) {
        return true;
    }
            ...
            <div className={className}>
                ...
                {days.map(weekDay => {
                    ...
                    const inPotentialRange = potentialRange
                        ? IsInPotentialRange(potentialRange, isoWeek, weekDay)
                        : false;

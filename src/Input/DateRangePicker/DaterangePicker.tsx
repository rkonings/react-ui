...
export const DateRangePresetsMenu = styled(
    ({ className, items, activePreset, onChange }: DateRangePresetsMenu) => {
        const [activeItem, setActiveItem] = React.useState<PresetItem | null>(
            null
        );
...
const DateDisplay = styled(
    ({ className, date, title, onClick }: DateDisplay) => {
        let formattedDate;
        if (date) {
...
const getDateRange = (item: Preset): DateRange | null => {
    if (isCurrent(item)) {
        ...
        return {
            start: moment()
                .subtract(item.value, item.type)
                .startOf(item.type),
            end: moment()
                .subtract(1, item.type)
                .endOf(item.type),
        };
    } else if (isLast(item)) {
        return {
...
export default styled(({ className, onChange }: DateRangePicker) => {
    ...
    const [activePreset, setActivePreset] = React.useState<PresetItem>(
        DATE_PRESET_ITEMS[0]
    );
    const [selectedPreset, setSelectedPreset] = React.useState<PresetItem>(
        DATE_PRESET_ITEMS[0]
    );
            ...
            <React.Fragment>
                ...
                <DatePickerMenu isOpen={isOpen}>
                    ...
                    {selectedPreset.action === 'CUSTOM' && (
                        <MenuFooter>
                            <TextButton onClick={() => onApplyCustomPreset()}>
                                Apply
                            </TextButton>
                        </MenuFooter>
                    )}
